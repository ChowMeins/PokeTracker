...
<div class='w-full h-full flex-grow overflow-y-auto'>
    <div class='w-full h-full px-8 flex flex-col'>
        <h1 class='py-8 text-center text-3xl font-semibold'> Account </h1>
        <div class='w-full h-full px-8 flex flex-col'>
            {#if userPhoto}
            <img src={userPhoto} alt="Profile" class='w-[15%] max-sm:mx-auto border-2 rounded-full'/>
            {:else} 
            ...
            <hr class='w-full border-1' />
                {#if userVerified}
                    <Checkmark className="" color="#43A047"/>
                {:else}
                    <div class='flex'>
                    <Close className="w-[48px]" color='#f45757' />
            ...
            <hr class='w-full border-1' />
            <p class='w-full flex max-sm:flex-col text-xl py-4'> {userEmail} <button class='w-fit ml-auto max-sm:ml-0 p-2 font-semibold bg-gray-500 rounded-xl' on:click={() => {displayModal('email-modal')}}> Change Email </button> </p>
            <h1 class='text-xl font-semibold py-2'> PASSWORD </h1>
            <hr class='w-full border-1' />
            <p class='w-full flex max-sm:flex-col text-xl py-4'> ************ <button class='w-fit ml-auto max-sm:ml-0 p-2 font-semibold bg-gray-500 rounded-xl' on:click={() => {
                const result = confirm("Send password reset email?");
                if (result && userEmail) {
                    authHandler.changePassword(auth, userEmail);
                }
            }}> Change Password </button> </p>
            <button class='w-fit mt-auto flex my-auto p-2 text-centertext-2xl font-semibold bg-gray-600 rounded-xl' on:click={authHandler.logout}> <Signout className="w-1/4 h-fit"/> <p class='w-full my-auto'>Logout</p> </button>
        </div>
    </div>
    <dialog id='email-modal' class='w-[400px] h-fit relative bg-[#0F141E] rounded-xl' on:close={() => {newEmail = "";}}>
        <div class='w-full h-full flex flex-col px-4 py-4'>
            <button class='w-fit ml-auto' on:click={() => {displayModal('email-modal')}}> <Close className='w-2/3 h-full' color=''/> </button>
